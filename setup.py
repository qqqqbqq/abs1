...
YELLOW = '\033[1;33m'
NORMAL = '\033[0m'
ABSMODE_PATH = '/'.join(os.path.abspath(__file__).split('/')[:-1])
...
class Installer:
    ...
    def installer(self):
        '''Install all AbsMode packages and modules'''
        ...
        if Config.get('actions', 'DEBUG', default=False):
            ...
            return
        tempPath = os.getcwd()
        run = f"python3 {os.path.abspath(os.path.join(ABSMODE_PATH, 'base/bin/run.py'))}"
        TOOLS_PATH = os.path.abspath(os.path.join(ABSMODE_PATH, 'base/tools'))
        try:
        ...
        try:
            ...
            try:
                with open(System.BASHRIC_FILE_PATH, "r") as f:
                    data = f.read()
                if data.find(System.ABSMODE_SHORTCUT.strip()) != -1:
                    with open(System.BASHRIC_FILE_PATH, "w") as f:
                        f.write(data.replace(System.ABSMODE_SHORTCUT.strip(), ""))
            except PermissionError:
                print(f"# {RED}cannot remove HackerMode shortcut!")
            print(f'# {GREEN}AbsMode installed successfully...')
    ...
    def update(self):
        if not Config.get('actions', 'DEBUG', cast=bool, default=False):
            os.system(
                f'cd {System.TOOL_PATH} && rm -rif {System.TOOL_NAME} && git clone https://github.com/qqqqbqq/{System.TOOL_NAME}')
            tempPath = os.getcwd()
            run = f"python3 {os.path.abspath(os.path.join(ABSMODE_PATH, 'base/bin/run.py'))}"
            TOOLS_PATH = os.path.abspath(os.path.join(ABSMODE_PATH, 'base/tools'))
            try:
...
if __name__ == '__main__':
    # tests:
    print('# To install write "python3 AbsMode install"')
